...
function searchUsers(query) {
    ...
    resultsDiv.innerHTML = '<div style="padding: 10px; text-align: center; color: #666;">Searching...</div>';
    ...
    searchTimeout = setTimeout(async () => {
        try {
            ...
            if (response.data.success && response.data.users) {
                ...
                resultsDiv.innerHTML = '<div style="padding: 10px; text-align: center; color: #666;">No users found</div>';
                resultsDiv.classList.add('show');
            }
        } catch (error) {
            console.error('User search error:', error);
            resultsDiv.innerHTML = '<div style="padding: 10px; text-align: center; color: #e74c3c;">Error searching users</div>';
            resultsDiv.classList.add('show');
        }
    }, 300);
}
...
function displayUserResults(users) {
    ...
    const html = users.map(user => {
        ...
        return `<div class="user-result-item" onclick='selectUser(${userJson})'>
...
function renderSelectedUsers() {
    ...
    const html = selectedUsers.map((user, index) => {
        const initials = user.username ? user.username.substring(0, 2).toUpperCase() : 'U';
        return `<div class="selected-user-item">
                    <div class="user-avatar">${initials}</div>
                    <div class="user-info">
                        <div class="user-name">${user.username || 'Unknown'}</div>
                        <div class="user-email">${user.email || ''}</div>
                    </div>
                    <button type="button" class="remove-user-btn" onclick="removeUser(${index})">×</button>
                </div>`;
    }).join('');
...
document.getElementById('couponForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        submitBtn.innerHTML = '<i class="fas fa-save me-2"></i> <span id="submitBtnText">Save Coupon</span>';
    }
});
        ...
        if (response.data.success) {
            ...
            content.innerHTML = `
                <div class="view-detail-row">
                    ...
                    <span class="coupon-code-display">${coupon.code}</span>
                </div>
                <div class="view-detail-row">
                    <strong>Name:</strong>
                ...
                <div class="view-detail-row">
                    <strong>Type:</strong>
                    <span>${coupon.type.replace('_', ' ').toUpperCase()}</span>
                </div>
                <div class="view-detail-row">
                ...
                <div class="view-detail-row">
                    ...
                    <span>₹${coupon.minPurchaseAmount.toLocaleString('en-IN')}</span>
                </div>
                <div class="view-detail-row">
                    <strong>Usage per User:</strong>
                    <span>${coupon.usageLimitPerUser === 0 ? 'Unlimited' : coupon.usageLimitPerUser}</span>
                </div>
                <div class="view-detail-row">
                    <strong>Total Usage:</strong>
                    <span>${coupon.currentUsageCount || 0} / ${coupon.totalUsageLimit === 0 ? 'Unlimited' : coupon.totalUsageLimit}</span>
                </div>
                <div class="view-detail-row">
                ...
                <div class="view-detail-row">
                    ...
                    <span>${new Date(coupon.startDate).toLocaleDateString('en-IN', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
                </div>
                <div class="view-detail-row">
                    <strong>Valid Until:</strong>
                    <span>${new Date(coupon.endDate).toLocaleDateString('en-IN', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
                </div>
                <div class="view-detail-row">
                ...
                <div class="view-detail-row">
                    ...
                    <span>${new Date(coupon.createdAt).toLocaleString('en-IN')}</span>
                </div>
            `;
